...
% approach with p constant. It is to simulate the behaviour of the asynchronous algorithm.
% The residual received by the slaves is different and the p
% vector is set to a constant value. Planar search is done after "kcount" updates which is given when the function is called.
...
%
% All the processors use g time-lagged residual in their computations.
...
% Even if the "deltasin" is set to 1, the delta values are not used to update
% the running residual since they are "incomplete" values. They are only used
% in the planar search phase.
