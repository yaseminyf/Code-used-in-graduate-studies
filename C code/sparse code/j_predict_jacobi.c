...
/* this version computes Jacobi  prediction                            */
...
#ifndef JACOBI
     i=residual->dim+nc+ngroup+1; /* arti 1 eklendi */
...
/* do the prediction */
         ...
         for (t=1;t<=antall;t++) {
           ...
           for (i=0;i<ngroup;i++) {
             buf_count=residual->dim+nc+ngroup+1; /* arti 1 eklendi */
...
printf("\n received from %d",status.MPI_SOURCE);
         for ( j=0;j<residual->dim;j++ ) {
           if (sol->ve[j]!=0.0) 
