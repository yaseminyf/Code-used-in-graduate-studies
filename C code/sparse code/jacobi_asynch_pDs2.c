...
/* for j=g,2g,3g updates p remains constant and after planar search    */
/* the queue of updates in the buffer at the master are  purged     !  */
/***********************************************************************/
...
/* update x vector with d_i vector */

         t=D[dest]->dim-ngroup+1;
         k=(dest-1)*nc;
         for (j=0;j<t;j++)
           x->ve[k+j]+=D[dest]->ve[dest-1+j];
              
/* update the residual with A_id_i */

         for ( j=0;j<residual->dim;j++ ) {
           if (Ad[dest]->ve[j]!=0.0)
             residual->ve[j]+=Ad[dest]->ve[j];
         } 
...
/* compute the columns of A*D=hatA matrix */
           ...
           for (k=1;k<=ngroup;k++) {
             for (j=0;j<Ad[k]->dim;j++) {
               Ad[k]->ve[j]+=Apptr[k-1]->ve[j]*deltav[k];
             }
             AD[k]=Ad[k];
           }     
...
/* if not converged then broadcast residual */
        ...
        if ((err < epsilon)||(r_norm==0.0)||(counter>=10000)) { /* final */
          ...
          if ( planeflag==0 ) {
            ...
            tag=3000;
            for (j=1;j<=ngroup;j++)
              MPI_Isend(&tag,1,MPI_INT,j,3000,MPI_COMM_WORLD,&req[j]);
            buf_count=200000;
            sndvector=CREATE(buf_count,char);
            position=0;
            j=residual->dim;
            ...
            tag=2000;
       /*     for (j=1;j<=ngroup;j++) {
              MPI_Isend(sndvector,position,MPI_PACKED,j,tag,MPI_COMM_WORLD,&request); 
            }     */
            MPI_Bcast(sndvector,position,MPI_PACKED,0,MPI_COMM_WORLD);  
            MPI_Barrier(MPI_COMM_WORLD);
          }
...
/* send s and sol with verdi to master */
      ...
      else if (status.MPI_TAG==/*2000*/3000) { /* new Aipi */
        MPI_Recv(&tag,1,MPI_INT,0,3000,MPI_COMM_WORLD,&status);
