...
/* j_pDs_seq_deneme.c.c = routine for solving the LSQ problem iteratively      */
...
/* garbage collection */
       ...
       s=gsslv(tildeA->n,xrnzptr[i],lnzv,xnzsubptr[i],nzsubptr[i],ddiag,s);
...
/* compute A*d */
       ...
       eind=Ad[i]->dim;
       for (j=0;j<eind;j++) {
         Ad[i]->ve[j]=sol->ve[indicesptr[i-1][j]-1];
       }
