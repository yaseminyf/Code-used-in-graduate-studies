...
printf("\n before loop...\n");
     ...
     while (converged==0) {
       for (i=1;i<=ngroup;i++ ) {
         ...
         buf_count=100000;
         position=0;
         MPI_Recv(&sndvector[0],buf_count,MPI_PACKED,MPI_ANY_SOURCE,500,MPI_COMM_WORLD,&status);
         dest=status.MPI_SOURCE;
         MPI_Unpack(&sndvector[0],buf_count,&position,&verdi,1,MPI_DOUBLE,MPI_COMM_WORLD);
         sol=v_get(residual->dim);
         MPI_Unpack(&sndvector[0],buf_count,&position,&sol->ve[0],residual->dim,MPI_DOUBLE,MPI_COMM_WORLD);
         s=v_get((int)(verdi));
         MPI_Unpack(&sndvector[0],buf_count,&position,&s->ve[0],s->dim,MPI_DOUBLE,MPI_COMM_WORLD);       
         DS[dest]=sol;
         D[dest]=s;
if ( dest == 1)
printf("\n sol1 %16.14e master \n",sol->ve[0]);
...
/* test for checking the effect of updated residual */
...
/* garbage collection */
       ...
       for (i=1;i<=ngroup;i++) {
         for ( j=0;j<residual->dim;j++ ) {
           if (DS[i]->ve[j]!=0.0) {
             residual->ve[j]+=DS[i]->ve[j]*omega;
             if ( choice1 >= '6' ) {
               sol->ve[j]+=DS[i]->ve[j]*omega;
             }
           }
         }  
       }
...
/* send both s and sol to master */
       ...
       buf_count=100000;
       position=0; 
       MPI_Pack(&verdi,1,MPI_DOUBLE,&sndvector[0],buf_count,&position,MPI_COMM_WORLD);
       j=sol->dim;
       MPI_Pack(&sol->ve[0],j,MPI_DOUBLE,&sndvector[0],buf_count,&position,MPI_COMM_WORLD);
       j=s->dim;
       MPI_Pack(&s->ve[0],j,MPI_DOUBLE,&sndvector[0],buf_count,&position,MPI_COMM_WORLD);
       MPI_Send(&sndvector[0],position,MPI_PACKED,0,tag,MPI_COMM_WORLD); 
if ( myid == 1 )
printf("\n sol1 %16.14e proc1 \n",sol->ve[0]);
#else
       buf_count=sol->dim;
