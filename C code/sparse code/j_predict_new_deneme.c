...
/* number of predictions and the residual */
     ...
     MPI_Pack(&antall,1,MPI_INT,buffer,buf_count,&position,MPI_COMM_WORLD);
     j=residual->dim;
     ...
     while (converged==0) {
       for (t=1;t<=antall;t++) {
         ...
         for (i=1;i<=ngroup;i++) {
           ...
           if (t<antall) {
             buf_count=vvec->dim;
             MPI_Send(&vvec->ve[0],buf_count,MPI_DOUBLE,dest,600,MPI_COMM_WORLD);   
           }
         } 
       }
...
/* if not converged then broadcast residual and A_ip_i*/
       ...
       if ((err < epsilon)||(r_norm==0.0)||(counter>15000)) { /* final */
         ...
         position=0;
         buf_count=residual->dim; /**(ngroup+1)+1; */
         buf_count=buf_count*sizeof(double); 
         buffer=CREATE(buf_count,char);
         j=1; /*residual->dim; */
         MPI_Pack(&residual->ve[0],j,MPI_DOUBLE,buffer,buf_count,&position,MPI_COMM_WORLD);
         MPI_Bcast(buffer,position /*buf_count */,MPI_PACKED,0,MPI_COMM_WORLD); 
         free((char *)(buffer));
       }
       else {
...
/* update x vector */
     ...
     MPI_Unpack(buffer,buf_count,&position,&antall,1,MPI_INT,MPI_COMM_WORLD);
     residual=v_get(A->m);
     j=residual->dim;
...
/* form tildeA */
     ...
     tildeA=createtildeA(tildeA,A,Apptr,ngroup,i,indicesptr);
...
/* get the nonzeros of sol */
         ...
         for (j=0;j<eind;j++) 
           sndvector[j+A->n]=sol->ve[indicesptr[myid-1][j]-1];
         tag=500;
         buf_count=A->n+eind;
