...
/* form the non-overlapping column groups of A */
     ...
     fp=fopen("pds_waitanytime","a");
     fprintf(fp,"\n file: %d ,  m: %d , n: %d, numslave: %d \n",choice1,A->m,A->n,numslave);
...
/* send the Aipi matrix and residual and nc */
...
/* create hatA matrix */
     ...
     while (converged==0) {
       ...
       for (i=1;i<=ngroup;i++ ) {
         ...
         endtime=MPI_Wtime();
         fprintf(fp,"waitany time: %16.14e seconds \n",endtime-starttime);
...
/* printf("\n residual norm: %16.14e\n",r_norm);           */
       ...
       if ((err < epsilon)||(r_norm==0.0)||(counter>15000)) {
         ...
         buf_count=residual->dim*(ngroup+1);                
         position=0;
         j=residual->dim;
         buf_count=buf_count*sfdouble; 
         buffer=CREATE(buf_count,char);
...
/* garbage collection */
     ...
     fprintf(fp,"\n total waitany time: %16.14e seconds \n",difftime); 
     fclose(fp);
   }
   else {     /*  slaves  */
...
/* probe for a final signal or new residual from master */
       ...
       if (residual->ve[0]==10000.0) {
         ...
         for (i=0;i<kcount;i++) {
           ...
           AA=sub_ptr[i];
           k=(myid-1)*kcount+i;
           putvaluesintildeA(tildeA,Apptr,AA,ngroup,k);
         } 
       }
     }
   }   /* end of slave */              
