...
/* this routine is for computing flop rate */
...
/* update the residual vector */       
     ...
     printf("\n Elapsed time master %16.14e seconds \n",difftime);
/*     count=(C->m*ngroup*(ngroup+1))+((ngroup*ngroup*ngroup+3*ngroup*ngroup-4*ngroup)/3);
     count+=(2*C->m*ngroup)+(2*ngroup*ngroup)+(2*C->m*ngroup); */
     count=2*C->m*ngroup*100;
     printf("flops master %d \n",count);
   }
   else { /* slaves */
...
/* begin the solution */
     ...
     printf("elapsed time slave %16.14e seconds \n",difftime);
     AA=sub_ptr[0];
     count=kcount*((4*AA->m*AA->n/kcount)+(2*AA->n*AA->n/(kcount*kcount)));
     printf("flops slave %d \n",count);
   }   /* end of slave */       
