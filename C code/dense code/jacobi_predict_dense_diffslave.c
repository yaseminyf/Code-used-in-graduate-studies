/* jacobi_predict_dense_diffslave.c                                   */
/* written 15.09.1999                                             */
/* last revised 17.09.1999                                        */
...
/* local variables */
   ...
   if ( myid == 0 ) {    /* master */
     ...
     sscanf(argv[4],"%d",&numslave);  /* get the number of slaves */ 
     switch(choice1) {
       case 1:
...
/* form the non-overlapping column groups of A */
     ...
     fp=fopen("res_predict_dense_diffslave","a");
...
/* send the Aipi matrix and residual and nc and choice3 */

     buf_count=((ngroup+1)*residual->dim+2)*sfdouble;
     buffer=CREATE(buf_count,char);
     position=0;
     j=residual->dim;
     MPI_Pack(&nc,1,MPI_INT,buffer,buf_count,&position,MPI_COMM_WORLD);
     MPI_Pack(&choice3,1,MPI_INT,buffer,buf_count,&position,MPI_COMM_WORLD);
...
/* update the residual vector and tmpApptr at the same time */     
...
/* get the number of groups to handle */
     ...
     Apptr=CREATE(ngroup,VECTOR *);
     buf_count=((ngroup+1)*sub_ptr[0]->m+2)*sfdouble;
     buffer=CREATE(buf_count,char);
     ...
     MPI_Unpack(buffer,buf_count,&position,&choice3,1,MPI_INT,MPI_COMM_WORLD);
...
/* form the tildeA matrices */ 
     ...
     for (i=0;i<kcount;i++) {
       ...
       t=(myid-1)*kcount+i;
       tildeA=createtildeA(tildeA,AA,Apptr,ngroup,t);
       tildeAptr[i]=tildeA;   
     }
...
/* begin the solution */
     ...
     converged=0;
     buf_countmain=nc+ngroup-1+residual->dim;
     sndvector=CREATE(buf_countmain,double);
     vvec=v_get(residual->dim);
     MPI_Barrier(MPI_COMM_WORLD); 
...
/* send both s and sol to master */
           ...
           if (t!=choice3) {
             ...
             buf_count=residual->dim*(ngroup+1)*sfdouble;
             buffer=CREATE(buf_count,char);
             MPI_Bcast(buffer,buf_count,MPI_PACKED,0,MPI_COMM_WORLD);
             position=0;
             j=residual->dim;
             ...
             for (i=0;i<kcount;i++) {
               ...
               AA=sub_ptr[i];
               l=(myid-1)*kcount+i;
               putvaluesintildeA(tildeA,Apptr,AA,ngroup,l);
